...
봉우리 지역이 몇개 있는 지 알아내는 프로그램을 작성하세요. 격자의 가장자리는 0으로 초기화 되었다고 가정한다.
...
list_M = [[0] * (N+2) for _ in range(N+2)]

for i in range(1, N+1):
    list_M[i][1:N+1] = list_N[i-1]
...
for i in range(1, N+1):
    for j in range(1, N+1):
        ...
        if tmp > left and tmp > right and tmp > up and tmp > down:
            cnt += 1
